...
void setup()
{
  ...
  m_sw.Begin(19200);
  m_sw.Println("xbee_api_sleep_txrx_usb_remote : ",
               version,
               debug::SOME);
...
void loop()
{
  ...
  {
    m_sw.Println("wireless timeout, going to sleep", debug::SOME);
    m_sleep_now = true;
  }
}
...
void handle_wireless()
{
  ...
  {
    m_sw.Print_msg("TX msg: ", tx_msg, DEC, debug::SOME);
...
void Transmit_frame(const uint8_t* frame, const uint8_t length)
{
  ...
  m_sw.Print_array("TX array: ", frame, length, HEX, debug::SOME);
  delay(10);
...
{
  m_sw.Print_msg("Message_received", rx_data, HEX, debug::SOME);

  switch(rx_data.payload_id)
  {
    case CMD_ID::ACK :
      m_sw.Println("Rx'd Ack", debug::SOME);
      m_sleep_now = true;
      break;

    case CMD_ID::IO_OUT :
      m_sw.Println("Rx'd IO out", debug::SOME);
      m_sleep_now = run_outputs(rx_data.payload);   // s3o3611
      break;

    default :
      m_sw.Print("Unknown CMD::ID: ", rx_data.payload_id, debug::SOME);
      m_sleep_now = true;
  }
